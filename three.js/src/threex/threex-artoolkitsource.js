...
ARjs.Source.prototype.init = function (onReady, onError) {
  ...
  this.parameters.sourceElement.style.position='relative';

  return this

  function onSourceReady() {
    _this.parameters.sourceElement.appendChild(_this.domElement);
    _this.ready = true
...
ARjs.Source.prototype.onResizeElement = function (mirrorDomElements) {
  var _this = this
  var screenWidth = _this.parameters.sourceElement.clientWidth
  var screenHeight = _this.parameters.sourceElement.clientHeight
  ...
  if (mirrorDomElements === undefined) {
    mirrorDomElements = []
  }
...
ARjs.Source.prototype.onResize = function (arToolkitContext, renderer, camera) {
  ...
  if (trackingBackend === 'artoolkit') {
    ...
    if (vrDisplay && vrDisplay.displayName === "Tango VR Device") {
      THREE.WebAR.resizeVRSeeThroughCamera(vrDisplay,
        camera)
    }
  } else {
    console.assert(false, 'unhandled trackingBackend ' + trackingBackend)
